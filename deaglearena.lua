...
local moduleloader = playerscripts:WaitForChild("ModuleLoader")
local deaglecontroller = require(moduleloader:WaitForChild("DeagleController"))
local viewmodelcontroller = require(moduleloader:WaitForChild("ViewmodelController"))
local cameracontroller = require(moduleloader:WaitForChild("CameraController"))
local movementcontroller = require(moduleloader:WaitForChild("MovementController"))
local crosshaircontroller = require(moduleloader:WaitForChild("CrosshairController"))
local effectscontroller = require(moduleloader:WaitForChild("EffectsController"))
local roundcontroller = require(moduleloader:WaitForChild("RoundController"))
local spectatecontroller = require(moduleloader:WaitForChild("SpectateController"))
local settings = require(moduleloader:WaitForChild("UI"):WaitForChild("Menus"):WaitForChild("Settings"))
local deagleshared = require(rs.Modules.Shared.DeagleShared)
local network = require(rs.Modules.Packages.Network)
local config = require(rs.Modules.Shared.Config)
local cases = require(rs.Modules.Shared.Config.Cases)
local skins = require(rs.Modules.Shared.Config.Skins)
local killeffs = require(rs.Modules.Shared.Config.KillEffects)
...
local sht_prsts = { deagle = 82286818216627, glock = 6581933860, tank = 138839154527248, ["tf2 shit"] = 124218436566507, custom = 0 }
...
local fov_col = rgb(100, 100, 255)
local aura_vis_col = rgb(255, 255, 0)
...
local st = {
    aim_target_part = nil, 
    aim_target_player = nil,
    silent_target_part = nil,
    silent_target_player = nil,
    lastClickTime = 0
}

local keyb = { aim = false, trigger = false }
local ws_en = false
local jp_en = false
local currtarg = nil
local cons = {}
...
local orig_ray = deagleshared.Raycast
deagleshared.Raycast = function(ignore, origin, target)
    if currtarg and flgs["silent_aim"] then
        local head = currtarg:FindFirstChild("Head") or currtarg:FindFirstChild("HumanoidRootPart")
        if head then
            local rp = RaycastParams.new()
            rp.FilterType = Enum.RaycastFilterType.Exclude
            rp.FilterDescendantsInstances = {getchar(), currtarg}
            local res = workspace:Raycast(origin, (head.Position - origin), rp)
            if res then
                return res
            end
            return {
    ...
    return orig_ray(ignore, origin, target)
end

local orig_onclientevent = network.OnClientEvent
network.OnClientEvent = function(self, name, func)
    if name == "Effect" then
        local hooked = function(...)
            local args = {...}
            if flgs["custom_kill_sounds"] and args[1] and args[1]:FindFirstChildOfClass("Humanoid") and args[1]:FindFirstChildOfClass("Humanoid").Health <= 0 then
                local id = 0
        ...
        return orig_onclientevent(self, name, hooked)
    end
    return orig_onclientevent(self, name, func)
end
...
local function run_killaura()
    while killaura_on do
        local root = getroot()
        if root then
            for _, char in ipairs(get_targs()) do
                ...
                if hum and head and hum.Health > 0 then
                    ...
                    if dist <= (flgs["kill_aura_range"] or 50) then
                        currtarg = char
                        pcall(function() deaglecontroller.Shoot() end)
                    end
                end
            end
        end
...
local function set_killaura(active)
    killaura_on = active
    if active then task.spawn(run_killaura) end
end

local hitbox_pts = {"Head", "UpperTorso", "LowerTorso", "HumanoidRootPart", "LeftUpperArm", "RightUpperArm", "LeftUpperLeg", "RightUpperLeg"}
local function expand_hb(char)
    local sz = flgs["hitbox_size"] or 8
    for _, name in ipairs(hitbox_pts) do
...
function get_targs()
    ...
    for _, obj in ipairs(ws:GetChildren()) do
        if obj:IsA("Model") and obj:FindFirstChild("Humanoid") and not plrs:GetPlayerFromCharacter(obj) and obj:GetAttribute("IsBot") then
...
local function get_aimtarg()
    local mp = uis:GetMouseLocation()
    local fov = flgs["aim_radius"] or 150
    ...
    for _, char in ipairs(get_targs()) do
        ...
        if h and part and hrp and h.Health > 0 then
            if root and (hrp.Position - root.Position).Magnitude > maxDist then continue end

            if flgs["aim_wall_check"] then
                local rp = RaycastParams.new()
            ...
            if screenDist < bestScore then best = char; bestScore = screenDist end
        end
    end
    return best
end
...
local function apply_vm_chams()
    local vm = viewmodelcontroller.GetViewmodel()
    if not vm then return end
    
    local function apply(part, color, material, trans, enabled)
        if not part or not part:IsA("BasePart") then return end
        if enabled then
            if not part:GetAttribute("lunxorig") then
                part:SetAttribute("lunxorig", true)
                part:SetAttribute("omat", part.Material)
                part:SetAttribute("ocol", part.Color)
                part:SetAttribute("otra", part.Transparency)
            end
            part.Color = color
            part.Material = material
            part.Transparency = trans
        else
            if part:GetAttribute("lunxorig") then
                part.Material = part:GetAttribute("omat")
                part.Color = part:GetAttribute("ocol")
                part.Transparency = part:GetAttribute("otra")
                part:SetAttribute("lunxorig", nil)
            end
        end
    end
...
for name, _ in pairs(cases) do
    table.insert(case_list, name)
end
table.sort(case_list)

local deagle_skins_list = {}
if skins then
    for name, _ in pairs(skins) do
        if name ~= "DefaultBeam" then
            table.insert(deagle_skins_list, name)
        end
    end
    table.sort(deagle_skins_list)
end

local killeff_list = {}
if killeffs then
    for name, _ in pairs(killeffs) do
        table.insert(killeff_list, name)
    end
    table.sort(killeff_list)
end

local radar_gui = Instance.new("ScreenGui")
radar_gui.Name = tostring(rng())
...
local aura_pt = Instance.new("Part")
aura_pt.Name = tostring(rng())
aura_pt.Anchored = true
aura_pt.CanCollide = false
aura_pt.Transparency = 0.5
aura_pt.Material = Enum.Material.Neon
aura_pt.Size = Vector3.new(1, 1, 1)
aura_pt.Parent = nil

local aura_msh = Instance.new("SpecialMesh", aura_pt)
aura_msh.MeshType = Enum.MeshType.Brick

local function set_aura_shp(shape)
    aura_msh.MeshId = ""
    if shape == "Circle" then
        aura_msh.MeshType = Enum.MeshType.Sphere
    elseif shape == "Cube" then
        aura_msh.MeshType = Enum.MeshType.Brick
    elseif shape == "Cylinder" then
        aura_msh.MeshType = Enum.MeshType.Cylinder
    elseif shape == "Diamond" then
        aura_msh.MeshType = Enum.MeshType.FileMesh
        aura_msh.MeshId = "rbxassetid://9756362"
    end
end

local wnd = lib:window({ name = "lunar x | deagle arena", size = UDim2.fromOffset(580, 600) })
...
local aim_sec = aim_tab:section({ name = "aimbot", side = "left" })
local sil_sec = aim_tab:section({ name = "silent aim", side = "left" })
local aim_cfg = aim_tab:section({ name = "config", side = "left" })
local fov_sec = aim_tab:section({ name = "fov circle", side = "right" })
local aura_sec = aim_tab:section({ name = "aura", side = "right" })

aim_sec:toggle({ name = "Aimbot", flag = "aim_on", default = false })
aim_sec:keybind({ name = "Aimbot Bind", flag = "aim_key", default = Enum.KeyCode.E, mode = "hold", callback = function(v) keyb.aim = v end })
aim_sec:toggle({ name = "Wall Check", flag = "aim_wall_check", default = true })
aim_sec:toggle({ name = "Trigger Bot", flag = "triggerbot", default = false })
aim_sec:keybind({ name = "Trigger Bind", flag = "trigger_key", default = Enum.KeyCode.Q, mode = "hold", callback = function(v) keyb.trigger = v end })
aim_sec:toggle({ name = "Auto Shoot", flag = "auto_shoott", default = false })
aim_sec:slider({ name = "Auto Shoot Delay", flag = "auto_shoot_delay", min = 0.01, max = 1, default = 0.05, interval = 0.01, suffix = "s" })
...
local fov_circle = newdraw("Circle", { Thickness = 1, Filled = false, Visible = false, Color = fov_col, Radius = 150 })
fov_sec:toggle({ name = "Visible", flag = "fov_visible", default = false, callback = function(v) if fov_circle then fov_circle.Visible = v end end })
fov_sec:colorpicker({ name = "FOV Color", flag = "fov_color", color = fov_col, alpha = 0, callback = function(c, t)
    fov_col = c
    if fov_circle then fov_circle.Color = c end
end})
fov_sec:slider({ name = "Radius", flag = "aim_radius", min = 10, max = 1000, default = 150, interval = 5, suffix = "px", callback = function(v) if fov_circle then fov_circle.Radius = v end end })

aura_sec:toggle({ name = "Kill Aura", flag = "kill_aura", default = false, callback = set_killaura })
aura_sec:slider({ name = "Kill Aura Range", flag = "kill_aura_range", min = 10, max = 500, default = 50, interval = 10, suffix = " st" })
aura_sec:toggle({ name = "Hitbox Expander", flag = "hitbox_on", default = false })
aura_sec:slider({ name = "Hitbox Size", flag = "hitbox_size", min = 1, max = 60, default = 8, interval = 0.5, suffix = " st" })
...
fx_sec:toggle({ name = "Kill Aura Visualizer", flag = "aura_vis", default = false })
fx_sec:dropdown({ name = "Aura Shape", flag = "aura_vis_shape", items = {"Circle", "Cube", "Cylinder", "Diamond"}, default = "Cylinder", callback = function(v) set_aura_shp(v) end })
fx_sec:colorpicker({ name = "Aura Vis Color", flag = "aura_vis_color", color = aura_vis_col, alpha = 0, callback = function(c) aura_vis_col = c end })
fx_sec:slider({ name = "Aura Transparency", flag = "aura_vis_trans", min = 0, max = 1, default = 0.5, interval = 0.1 })
fx_sec:dropdown({ name = "Aura Material", flag = "aura_vis_mat", items = mats, default = "Neon" })
fx_sec:dropdown({ name = "Aura Position", flag = "aura_vis_pos", items = {"Head", "HumanoidRootPart", "LeftUpperLeg", "RightUpperLeg"}, default = "HumanoidRootPart" })
fx_sec:slider({ name = "Aura X Offset", flag = "aura_vis_x", min = -10, max = 10, default = 0, interval = 0.5 })
fx_sec:slider({ name = "Aura Y Offset", flag = "aura_vis_y", min = -10, max = 10, default = 0, interval = 0.5 })
fx_sec:slider({ name = "Aura Thickness", flag = "aura_vis_thick", min = 0.1, max = 10, default = 0.1, interval = 0.1 })
...
wpn_main_sec:toggle({ name = "No Shoot Delay", flag = "no_shoot_delay", default = false })
wpn_main_sec:toggle({ name = "No Recoil", flag = "no_recoil", default = false, callback = function(v)
    if v then
        cameracontroller.Shoot = function() end
    else
        cameracontroller.Shoot = function() 
            cameracontroller.u2:AddVelocity(0.017)
        end
    end
end})

local case_sec = wpn_tab:section({ name = "cases", side = "right" })
local case_drp = case_sec:dropdown({ name = "Select Case", flag = "case_select", items = case_list, default = case_list[1] })
case_sec:button({ name = "Open Case", callback = function()
    pcall(function() network:InvokeServer("OpenCase", flgs["case_select"]) end)
    pcall(function() network:FireServer("OpenCase", flgs["case_select"]) end)
end})
case_sec:toggle({ name = "Auto Open Cases", flag = "auto_open_cases", default = false, callback = function(v)
    if v then
        task.spawn(function()
            while flgs["auto_open_cases"] do
                pcall(function() network:InvokeServer("OpenCase", flgs["case_select"]) end)
                pcall(function() network:FireServer("OpenCase", flgs["case_select"]) end)
                task.wait(1)
            end
...
local skn_sec = skn_tab:section({ name = "skin changer", side = "left" })
local skn_drp = skn_sec:dropdown({ name = "Deagle Skin", flag = "deagle_skin", items = deagle_skins_list, default = "Default Deagle" })
skn_sec:button({ name = "Apply Deagle Skin", callback = function()
    if viewmodelcontroller then
        local char = getchar()
        if char then
            local deagle = char:FindFirstChild("Deagle")
            if deagle then
                pcall(function() deagle:SetAttribute("DeagleSkinName", flgs["deagle_skin"]) end)
            end
        end
        pcall(function() viewmodelcontroller.SetDeagle(flgs["deagle_skin"]) end)
        lib:notification({text = "applied deagle: ".. (flgs["deagle_skin"] or "Unknown")})
    end
end})

local eff_drp = skn_sec:dropdown({ name = "Kill Effect", flag = "kill_effect", items = killeff_list, default = "Knock" })
skn_sec:button({ name = "Apply Kill Effect", callback = function()
    if network then
        pcall(function() network:FireServer("SetKillEffect", flgs["kill_effect"]) end)
        pcall(function() lp:SetAttribute("KillEffect", flgs["kill_effect"]) end)
        lib:notification({text = "applied kill effect: ".. (flgs["kill_effect"] or "Unknown")})
    end
end})
...
misc_sec:toggle({ name = "Auto Deploy", flag = "auto_deploy", default = false, callback = function(v)
    if v then
        task.spawn(function()
            while flgs["auto_deploy"] do 
				if lp:GetAttribute("InRound") == false then
					pcall(function() roundcontroller.Spawn() end)
                    task.wait(1)
				else
					task.wait(1)
				end
 ...
 misc_sec:toggle({ name = "Hide the Leave gui", flag = "i_hate_the_leave_gui", default = false, callback = function(v)
    if v then
        task.spawn(function()
            while task.wait(1) do
                local leavegui = game:GetService("Players").LocalPlayer.PlayerGui.MainGui.LeavingGui
                if leavegui.Visible == true then
...
misc_sec:toggle({ name = "Third Person", flag = "third_person", default = false, callback = function(v)
    if v then
	    ...
	    if lp:GetAttribute("InRound") == true then
			lp.CameraMaxZoomDistance = 0 -- tung tung
		else
			lp.CameraMaxZoomDistance = 128
		end
    end
...
snd_sec:toggle({ name = "Custom Shoot Sounds", flag = "custom_shoot_sounds", default = false })
snd_sec:dropdown({ name = "Shoot Sound", flag = "shoot_sound_preset", items = {"deagle", "glock", "tank", "tf2 shit", "custom"}, default = "deagle" })
snd_sec:textbox({ name = "Custom Shoot Sound ID", flag = "custom_shoot_sound_id", placeholder = "rbxassetid://" })
...
ui_sec:button({ name = "Unload", callback = function()
    ...
    deagleshared.Raycast = orig_ray
    network.OnClientEvent = orig_onclientevent
    cameracontroller.Shoot = function() 
        cameracontroller.u2:AddVelocity(0.017)
    end
...
connct(run.RenderStepped, function(dt)
    ...
    local currentAimTarget = get_aimtarg()
    st.aim_target_player = currentAimTarget
    st.aim_target_part = currentAimTarget and currentAimTarget:FindFirstChild(flgs["aim_part"] or "Head") or nil
    ...
    local death_vis = false
    local maingui = pgui:FindFirstChild("MainGui")
    if maingui then
        local deathscreen = maingui:FindFirstChild("HUD") and maingui.HUD:FindFirstChild("DeathCameraGui")
        if deathscreen and deathscreen.Visible then
            death_vis = true
        end
    end
    ...
    if death_vis then
        ...
        local c = getchar()
        local deployed = c and c:GetAttribute("InRound")
    ...
    if flgs["aim_on"] and keyb.aim and st.aim_target_part then
        local smooth = flgs["aim_smooth"] or 0.15
        cam.CFrame = cam.CFrame:Lerp(CFrame.lookAt(cam.CFrame.Position, st.aim_target_part.Position), smooth)
    end

    if flgs["triggerbot"] and keyb.trigger and st.aim_target_player and tick() - trig_cd > 0.1 then
        mouse1press(); task.wait(0.02); mouse1release(); trig_cd = tick()
    end

    if flgs["auto_shoott"] and tick() - auto_shoot_cd > (flgs["auto_shoot_delay"] or 0.05) then
        pcall(function() deaglecontroller.Shoot() end)
        auto_shoot_cd = tick()
    end

    if flgs["fov_visible"] and fov_circle then
        fov_circle.Visible = true
        fov_circle.Position = Vector2.new(mp.X, mp.Y)
        fov_circle.Color = fov_col
        fov_circle.Thickness = 1
        fov_circle.Radius = flgs["aim_radius"] or 150
    else
    ...
    if flgs["aura_vis"] and killaura_on then
        local char = getchar()
        if char then
            local p = char:FindFirstChild(flgs["aura_vis_pos"] or "HumanoidRootPart")
            if p then
                aura_pt.Color = aura_vis_col
                aura_pt.Transparency = flgs["aura_vis_trans"] or 0.5
                aura_pt.Material = Enum.Material[flgs["aura_vis_mat"] or "Neon"]
                
                local range = flgs["kill_aura_range"] or 50
                local thick = flgs["aura_vis_thick"] or 0.1
                local offset = Vector3.new(flgs["aura_vis_x"] or 0, flgs["aura_vis_y"] or 0, 0)
                local curShape = flgs["aura_vis_shape"] or "Circle"
                
                if curShape == "Cylinder" then
                    aura_msh.Scale = Vector3.new(thick, range*2, range*2)
                    aura_pt.CFrame = CFrame.new(p.Position + offset) * CFrame.Angles(0, 0, math.rad(90))
                else
                    aura_msh.Scale = Vector3.new(range*2, range*2, range*2)
                    aura_pt.CFrame = CFrame.new(p.Position + offset)
                end
    ...
    for _, char in ipairs(get_targs()) do
        ...
        apply_chams(char)
        if flgs["hitbox_on"] then expand_hb(char) end
